...
(function (TcAdsWebService) {
    class Response {
        constructor(hasError, error, reader) {
            this.hasError = hasError;
            this.error = error;
            this.reader = reader;
        }
    }
    TcAdsWebService.Response = Response;
    class InternalError {
    ...
    TcAdsWebService.InternalError = InternalError;
    class ResquestError {
        constructor(requestStatus, requestStatusText) {
            this.requestStatus = requestStatus;
            this.requestStatusText = requestStatusText;
        }
    }
    TcAdsWebService.ResquestError = ResquestError;
    class Client {
        constructor(sServiceUrl, sServiceUser, sServicePassword) {
            this.sServiceUrl = sServiceUrl;
            this.sServiceUser = sServiceUser;
            this.sServicePassword = sServicePassword;
        }
        readwrite(sNetId, nPort, nIndexGroup, nIndexOffset, cbRdLen, pwrData, pCallback, userState, ajaxTimeout, errorCallback) {
        ...
        readState(sNetId, nPort, pCallback, userState, ajaxTimeout, errorCallback) {
            const message = '<?xml version="1.0" encoding="utf-8"?>' +
                '<SOAP-ENV:Envelope ' +
                'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance/" ' +
                'xmlns:xsd="http://www.w3.org/2001/XMLSchema/" ' +
        ...
        writeControl(sNetId, nPort, adsState, deviceState, pData, pCallback, userState, ajaxTimeout, errorCallback) {
            const message = '<?xml version="1.0" encoding="utf-8"?>' +
                '<SOAP-ENV:Envelope ' +
                'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance/" ' +
                'xmlns:xsd="http://www.w3.org/2001/XMLSchema/" ' +
        ...
        write(sNetId, nPort, nIndexGroup, nIndexOffset, pData, pCallback, userState, ajaxTimeout, errorCallback) {
            const message = '<?xml version="1.0" encoding="utf-8"?>' +
                '<SOAP-ENV:Envelope ' +
                'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance/" ' +
                'xmlns:xsd="http://www.w3.org/2001/XMLSchema/" ' +
        ...
        read(sNetId, nPort, nIndexGroup, nIndexOffset, cbLen, pCallback, userState, ajaxTimeout, errorCallback) {
            const message = '<?xml version="1.0" encoding="utf-8"?>' +
                '<SOAP-ENV:Envelope ' +
                'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance/" ' +
                'xmlns:xsd="http://www.w3.org/2001/XMLSchema/" ' +
        ...
        handleResponse(response) {
            ...
            if (faultstringNodes.length !== 0) {
                ...
                const resp = new TcAdsWebService.Response(true, new TcAdsWebService.InternalError(errorMessage, errorCode), undefined);
                response = null;
                return resp;
            }
            else {
                ...
                if (soapData) {
                    const resp = new TcAdsWebService.Response(false, undefined, new TcAdsWebService.DataReader(soapData));
                    response = null;
                    return resp;
                }
                else {
        ...
        sendMessage(message, method, pCallback, userState, ajaxTimeout, errorCallback) {
            axios_1.default.post(this.sServiceUrl, message, {
                headers: {
                ...
                .then((response) => {
                pCallback(this.handleResponse(response), userState);
            })
                .catch((err) => {
                if (errorCallback) {
                    errorCallback(err);
    ...
    class DataReader {
        ...
        readREAL() {
            ...
            const res = binary2Real(binStr, TcAdsWebService.TcAdsWebServiceDataTypes.REAL);
            this.offset = this.offset + 4;
            return res;
        }
        readLREAL() {
            ...
            const res = binary2Real(binStr, TcAdsWebService.TcAdsWebServiceDataTypes.LREAL);
            this.offset = this.offset + 8;
            return res;
        }
    }
    TcAdsWebService.DataReader = DataReader;
    class DataWriter {
        constructor() {
            this.byteArray = [];
        }
        ...
        writeSINT(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.Integer, 1, this.byteArray);
        }
        writeINT(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.Integer, 2, this.byteArray);
        }
        writeDINT(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.Integer, 4, this.byteArray);
        }
        writeBYTE(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.UnsignedInteger, 1, this.byteArray);
        }
        writeWORD(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.UnsignedInteger, 2, this.byteArray);
        }
        writeDWORD(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.UnsignedInteger, 4, this.byteArray);
        }
        writeBOOL(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.BOOL, 1, this.byteArray);
        }
        writeString(value, length) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.String, length, this.byteArray);
        }
        writeREAL(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.REAL, 4, this.byteArray);
        }
        writeLREA(value) {
            this.byteArray = this.PrepareData(value, TcAdsWebService.TcAdsWebServiceDataTypes.LREAL, 8, this.byteArray);
        }
        PrepareData(data, type, len, array) {
            let j = array.length;
    ...
    TcAdsWebService.DataWriter = DataWriter;
    TcAdsWebService.TcAdsReservedIndexGroups = {
        'PlcRWMX': 16416,
        'PlcRWMB': 16416,
        'PlcRWRB': 16432,
        'PlcRWDB': 16448,
    ...
    TcAdsWebService.TcAdsWebServiceDataTypes = {
        'String': 0,
        'BOOL': 1,
        'Integer': 2,
        'UnsignedInteger': 3,
    ...
    function byteArrayToBinaryString(arr) {
        let res = '';
        arr.forEach(item => res += String.fromCharCode(item & 0xFF));
        return res;
    }
    class Base64 {
        static encode(data) {
    ...
    Base64.tab = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
    function real2Binary(value, type) {
        let exp;
        let man;
        let bias;
        switch (type) {
    ...
    function binary2Real(binary, type) {
        let nullE = true;
        let nullF = true;
        let oneE = true;
        let strE = '';
    ...
    function convertDataToUInt(data, len) {
        let res = 0;
        if (len === 4) {
    ...
    function convertDataToInt(data, len) {
        let res = 0;
        if (len === 4) {
